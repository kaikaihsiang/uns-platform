...
@router.put("/{schema_id}/restore", response_model=SchemaTypeOut)
...
@router.get("/{schema_id}", response_model=SchemaTypeOut)
...
async def create_payload_schema(body: SchemaTypeCreate, db: AsyncSession = Depends(get_db)):
    """建立新 Schema Type。"""
...
@router.put("/{schema_id}", response_model=SchemaTypeOut)
async def update_schema_type(
    schema_id: int, body: SchemaTypeUpdate, db: AsyncSession = Depends(get_db)
):
    """更新 Schema Type。"""
    try:
...
@router.post("/{schema_id}/approve", response_model=SchemaTypeOut)
